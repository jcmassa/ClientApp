const RankingGrid = ({ items, imgArr, drag, allowDrop, drop }) =>
{
    ...
    function pushCellMarkupToArr(cellCollection, rankNum, rowLabel)
    {
        ...
        {
            cellCollection.push(<div className="row-label" id={rowLabel.replace(/\s/g, '')}><h4 id={rowLabel.replace(/\s/g, '') + "lbl"}>{rowLabel}</h4></div>)
        }
    }
    ...
    {
        rankingGrid.push(<div className="rank-row top-tier"    id='topTierRow'>{cellCollectionTop}</div>);
        rankingGrid.push(<div className="rank-row middle-tier" id='middleTierRow'>{cellCollectionMiddle}</div>);
        rankingGrid.push(<div className="rank-row bottom-tier" id='bottomTierRow'>{cellCollectionBottom}</div>);
        rankingGrid.push(<div className="rank-row worst-tier"  id='worstTierRow'>{cellCollectionWorst}</div>);
        return rankingGrid;
    }
    ...
    return (
        <div className = "rankings" id="rankingBase">
            { createRankingGrid() }
        </div>
    )
}
